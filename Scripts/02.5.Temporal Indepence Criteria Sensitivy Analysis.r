...
##Looping over each independence criteria
	for (zz in 1:length(criteria.vec)){
		...
		z<-criteria.vec[zz]	#Selecting zzth independence criterion
		
		##Determining temporally independent records
		fotos.ind.ls[[zz]]<-ind_record(
			#Vector of species' names
			sp=fotos$especie,
			#Vector of independence criteria
			z=z,
...
###
	...
	##Loop over all 'Dubious Records' 
		for (i in dubious.records){
			...
			###from any other independet individual
